...
# Load JSON file
with open('examples/flapper/circular_traj/circular_Z_20250423190018.json', 'r') as f:
    data = json.load(f)
...
cmd_roll = np.array([c["controller.cmd_roll"] for c in controller_cmd]) / 180 * math.pi # [rad]?
cmd_pitch = np.array([c["controller.cmd_pitch"] for c in controller_cmd]) / 180 * math.pi # [rad]?
cmd_yaw = np.array([c["controller.cmd_yaw"] for c in controller_cmd]) / 180 * math.pi # [rad]?

# Plot attitude
stabilizer = data["stabilizer"] # [deg]
state_roll = np.array([c["stabilizer.roll"] for c in stabilizer]) / 180 * math.pi # [rad]
state_pitch = np.array([c["stabilizer.pitch"] for c in stabilizer]) / 180 * math.pi # [rad]
state_yaw = np.array([c["stabilizer.yaw"] for c in stabilizer]) / 180 * math.pi # [rad]

controller_attitude = data["controller_attitude"] # [deg]
controller_roll = np.array([c["controller.roll"] for c in controller_attitude]) / 180 * math.pi # [rad]
controller_pitch = np.array([c["controller.pitch"] for c in controller_attitude]) / 180 * math.pi # [rad]
controller_yaw = np.array([c["controller.yaw"] for c in controller_attitude]) / 180 * math.pi # [rad]
...
roll_rate = np.array([c["stateEstimateZ.rateRoll"] for c in attitude_rate]) / 1000 #[rad/s]
pitch_rate = np.array([c["stateEstimateZ.ratePitch"] for c in attitude_rate]) / 1000 #[rad/s]
yaw_rate = np.array([c["stateEstimateZ.rateYaw"] for c in attitude_rate]) / 1000 #[rad/s]

roll_rate_deriv = np.gradient(state_roll, dt)
pitch_rate_deriv = np.gradient(state_pitch, dt)
yaw_rate_deriv = np.gradient(state_yaw, dt)

controller_attitude_rate = data["controller_attitude_rate"] #[deg/s]
controller_roll_rate = np.array([c["controller.rollRate"] for c in controller_attitude_rate]) / 180 * math.pi #[rad/s]
controller_pitch_rate = np.array([c["controller.pitchRate"] for c in controller_attitude_rate]) / 180 * math.pi #[rad/s]
controller_yaw_rate = np.array([c["controller.yawRate"] for c in controller_attitude_rate]) / 180 * math.pi #[rad/s]
